...
describe('TarjetaService', () => {
  ...
  describe('validateTarjetaData', () => {
    test('should validate required fields', () => {
      const invalidData = {};
      const errors = tarjetaService.validateTarjetaData(invalidData);

      expect(errors).toContain('El nombre de la tarjeta es requerido');
      expect(errors).toContain('El banco es requerido');
      expect(errors).toContain('El tipo de tarjeta es requerido');
    });
    ...
    test('should validate invalid card type', () => {
      const invalidData = {
        nombre: 'Test Card',
        banco: 'Test Bank',
        tipo: 'invalid_type'
      ...
      expect(errors).toContain('Tipo de tarjeta inválido. Debe ser: debito, credito, virtual');
    });

    test('should validate credit card requires closing date', () => {
      const invalidData = {
        nombre: 'Credit Card',
        banco: 'Bank',
        tipo: 'credito'
      ...
      expect(errors).toContain('Las tarjetas de crédito requieren día de cierre');
      expect(errors).toContain('Las tarjetas de crédito requieren día de vencimiento');
    });

    test('should validate credit card date ranges', () => {
      const invalidData = {
        nombre: 'Credit Card',
        banco: 'Bank',
        tipo: 'credito',
        dia_mes_cierre: 0,
        dia_mes_vencimiento: 32
      };
      const errors = tarjetaService.validateTarjetaData(invalidData);

      // Since day 0 is treated as falsy, it triggers the "required" error instead of range error
      expect(errors).toContain('Las tarjetas de crédito requieren día de cierre');
      expect(errors).toContain('El día de vencimiento debe estar entre 1 y 31');
    });

    test('should validate debit cards should not have dates', () => {
      const invalidData = {
        nombre: 'Debit Card',
        banco: 'Bank',
        tipo: 'debito',
      ...
      expect(errors).toContain('Las tarjetas de débito no deben tener días de cierre o vencimiento');
    });

    test('should pass validation for valid credit card', () => {
      const validData = {
        nombre: 'Visa Credit',
        banco: 'Banco Nacion',
        tipo: 'credito',
    ...
    test('should pass validation for valid debit card', () => {
      const validData = {
        nombre: 'Visa Debit',
        banco: 'Banco Nacion',
        tipo: 'debito'
