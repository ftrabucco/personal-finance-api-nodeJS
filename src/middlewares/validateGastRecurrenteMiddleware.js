...
const gastoRecurrenteSchema = Joi.object({
  ...
  frecuencia_gasto_id: Joi.number()
    .integer()
    .required()
    .min(1)
    .messages({
      'number.base': 'La frecuencia debe ser un número',
      'number.empty': 'La frecuencia es requerida',
      'number.min': 'La frecuencia debe ser un número válido'
    }),
...
export function validateCreateGastoRecurrente(req, res, next) {
  const { error } = gastoRecurrenteSchema.validate(req.body, { abortEarly: false });
  if (error) {
